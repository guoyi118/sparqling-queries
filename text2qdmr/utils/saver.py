...
def save_checkpoint(items, step, model_dir, ignore=[],
                    keep_every_n=10000000, max_keep=100):
    ...
    if keep_every_n is not None:
        ...
        last_step = float('-inf')
        all_checkpoints_pruned = []
        for checkpoint_step, name in all_checkpoints:
            if checkpoint_step - last_step >= keep_every_n or checkpoint_step % keep_every_n == 0:
                last_step = checkpoint_step
                all_checkpoints_pruned.append((checkpoint_step, name))
                continue
            os.unlink(os.path.join(model_dir, name))

        if max_keep is not None and len(all_checkpoints_pruned) > max_keep:
            for _, name in all_checkpoints_pruned[:-max_keep]:
                os.unlink(os.path.join(model_dir, name))

class Saver(object):
    """Class to manage save and restore for the model and optimizer."""

    def __init__(self, items, keep_every_n=None, max_keep=None):
    ...
    def save(self, model_dir, step):
        ...
        save_checkpoint(self._items, step, model_dir,
                        keep_every_n=self._keep_every_n, max_keep=self._max_keep)
